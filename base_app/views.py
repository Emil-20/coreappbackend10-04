...
def Newtrainees(request):
    if 'usernametm2' in request.session:
        ...
        memm = user_registration.objects.filter(
            designation_id=mem1).order_by('-id')
...
def newtraineeesteam(request):
    if 'usernametm2' in request.session:
        ...
        if request.method == 'POST':
            ...
            register.team = create_team.objects.get(
                id=int(request.POST['team']))
            register.total_amount = course.objects.get(
                id=int(request.POST['cou'])).total_fee
            register.department = department.objects.get(
                id=int(request.POST['dept']))
            users = previousTeam()
            users.teamname = create_team.objects.get(
                id=int(request.POST['team']))
    ...
    if 'usernametm2' in request.session:
        ...
        if request.method == 'POST':
            ...
            attend = attendance.objects.filter(
                date__gte=start, date__lte=end, user_id=user)
...
def Trainers_Attendancetable(request):
    if 'usernametm2' in request.session:
        ...
        if request.method == 'POST':
            ...
            attend = attendance.objects.filter(
                date__gte=start, date__lte=end, user_id=user).order_by('-id')
...
def Trainee(request):
    if 'usernametm2' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernametm).filter(fullname=usernametm1)
        des = designation.objects.get(designation='trainee')
        tre = user_registration.objects.filter(
            designation=des.id).all().order_by('-id')
...
def trainerunsolvedissue(request):
    if 'usernametm2' in request.session:
        ...
        cut = reported_issue.objects.filter(
            reported_to_id=usernametm2, designation_id=des.id, issuestatus=0)
        a = cut.count()
...
def reportedissuesub(request):
    if 'usernametm2' in request.session:
        ...
        cut = reported_issue.objects.filter(
            reporter=usernametm2).order_by('-id')
        context = {'cut': cut, 'vars': vars, 'mem': mem}
...
def trainers_leavelist(request):
    if 'usernametm2' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernametm) .filter(fullname=usernametm1)
        des = designation.objects.get(designation='trainer')
        cut = leave.objects.filter(designation_id=des.id).filter(
            leaveapprovedstatus=0).order_by('-id')
...
def current(request, id):
    if 'usernametm2' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernametm) .filter(fullname=usernametm1)
        d = user_registration.objects.get(id=id)
        tm = create_team.objects.filter(trainer=d.fullname).filter(
...
def Trainer_Currenttrainee(request, id):
    if 'usernametm2' in request.session:
        ...
        des = designation.objects.get(designation='trainee')
        vars = user_registration.objects.filter(
            team=d.id, designation=des.id).order_by('-id')
    ...
    if 'usernametm2' in request.session:
        ...
        if request.method == 'POST':
            ...
            atten = attendance.objects.filter(
                date__gte=std, date__lte=edd, user_id=user)
...
def Previous(request, id):
    if 'usernametm2' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernametm) .filter(fullname=usernametm1)
        d = user_registration.objects.get(id=id)
        tm = create_team.objects.filter(
            trainer=d.fullname).filter(team_status=1)
        des = designation.objects.get(designation='trainer')
        cut = user_registration.objects.filter(designation_id=des.id)
        vars = user_registration.objects.filter(
            designation_id=usernametm).filter(fullname=usernametm1)
...
def PList(request, id):
    if 'usernametm2' in request.session:
        ...
        if request.method == 'POST':
            ...
            user = vars
            atten = attendance.objects.filter(
                date__gte=std, date__lte=edd, user_id=user).order_by('-id')
...
def trainer_myreportissue_table(request):
    if 'usernametrnr2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernametrnr2)
        rm = reported_issue.objects.filter(
            reporter_id=usernametrnr2).order_by('-id')
...
def trainer_addtopic(request):
    if 'usernametrnr2' in request.session:
        ...
        crt = create_team.objects.filter(trainer=uname.fullname)
        mem = topic()
        if request.method == 'POST':
...
def trainer_attendance_trainer_viewattendancelist(request):
    if 'usernametrnr2' in request.session:
        ...
        if request.method == 'POST':
            ...
            atten = attendance.objects.filter(
                date__gte=std, date__lte=edd, user_id=usernametrnr2).order_by('-id')
...
def trainer_currentteam(request):
    if 'usernametrnr2' in request.session:
        ...
        tm = create_team.objects.filter(trainer=usernametrnr1).filter(
            team_status=0).order_by('-id')
...
def trainer_previoustrainees(request, id):
    if 'usernametrnr2' in request.session:
        ...
        des = designation.objects.get(designation='trainee')
        mem = user_registration.objects.filter(
            designation_id=des.id).filter(team_id=d)
...
def trainer_teamlistpage(request):
    if 'usernametrnr2' in request.session:
        ...
        tam = create_team.objects.filter(
            trainer=usernametrnr1, team_status=0).order_by('-id')
        des = designation.objects.get(designation='trainee')
        cut = user_registration.objects.filter(designation_id=des.id)
...
def trainer_givetask(request, id):
    if 'usernametrnr2' in request.session:
        ...
        var = user_registration.objects.filter(
            team_id=d).filter(designation_id=des.id)
        print(var)
...
def trainer_taskgivenpage(request, id):
    if 'usernametrnr2' in request.session:
        ...
        mem1 = user_registration.objects.filter(
            designation_id=des.id).filter(team_id=d).order_by('-id')
        mem = user_registration.objects.filter(
            designation_id=des.id).filter(team_id=d).values_list('id')
        tsk = trainer_task.objects.filter(team_name_id=d.id).filter(
            user_id__in=mem).order_by('-id')
...
def trainer_task_completed_teamlist(request):
    if 'usernametrnr2' in request.session:
        ...
        tam = create_team.objects.filter(
            trainer=usernametrnr1, team_status=1).order_by('-id')
        des = designation.objects.get(designation='trainee')
        cut = user_registration.objects.filter(designation_id=des.id)
...
def trainer_current_trainees(request):
    if 'usernametrnr2' in request.session:
        ...
        z = user_registration.objects.filter(
            fullname=usernametrnr1, id=usernametrnr2)
        cut = create_team.objects.filter(
            trainer=usernametrnr1).values_list('id', flat=True)
        print(cut)
        des = designation.objects.get(designation='trainee')
        user = user_registration.objects.filter(
            designation_id=des.id, team_id__in=cut)
...
def trainer_paymentlist(request):
    if 'usernametrnr2' in request.session:
        ...
        z = user_registration.objects.filter(designation_id=usernametrnr) .filter(
            fullname=usernametrnr1) .filter(id=usernametrnr2)
        mem = acntspayslip.objects.filter(
            user_id=usernametrnr2).all().order_by('-id')
...
def trainer_attendance_trainees_viewattendance(request):
    if 'usernametrnr2' in request.session:
        ...
        des = designation.objects.get(designation='trainee')
        vars = user_registration.objects.filter(
            designation=des.id, team_id__in=cut)
...
def trainee_currentTopic(request):
    if 'usernametrns2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernametrns2)
        mem = topic.objects.filter(team_id=usernametrns3) .filter(
            topic_status=0).order_by('-id')
...
def trainee_previousTopic(request):
    if 'usernametrns2' in request.session:
        ...
        if request.session.has_key('usernametrns3'):
            ...
            return redirect('/')
        z = user_registration.objects.filter(id=usernametrns2)
        mem = topic.objects.filter(
            team_id=usernametrns3).filter(topic_status=1)
...
def trainee_task_list(request):
    if 'usernametrns2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernametrns2)
        mem = trainer_task.objects.filter(
            user_id=usernametrns2).filter(task_status=0).order_by('-id')
...
def trainee_completed_taskList(request):
    if 'usernametrns2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernametrns2)
        mem = trainer_task.objects.filter(user_id=usernametrns2).filter(
...
def trainee_reported_issue(request):
    if 'usernametrns2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernametrns2)
        n = reported_issue.objects.filter(
            reporter_id=usernametrns2).order_by('-id')
...
def trainee_report_reported(request):
    if 'usernametrns2' in request.session:
        ...
        z = user_registration.objects.filter(
            designation_id=usernametrns).filter(id=usernametrns2)
        var = reported_issue()
        if request.method == 'POST':

            # ree= user_registration.objects.get(designation_id=mem1)
            var.designation_id = usernametrns
            var.reported_to = user_registration.objects.get(
                id=int(request.POST['reportto']))
...
def trainees_attendance_viewattendancelist(request):
    if 'usernametrns2' in request.session:
        ...
        if request.method == 'POST':
            ...
            user = usernametrns2
            vars = attendance.objects.filter(
                date__gte=start, date__lte=end, user_id=user)
            ...
            if oldps == cur:
                if oldps != newps:
                    ...
                    messages.add_message(
                        request, messages.INFO, 'Current and New password same')
                else:
                    messages.info(request, 'Incorrect password same')
                ...
                messages.add_message(
                    request, messages.INFO, 'old password wrong')
...
def MAN_projectman(request, id, did):
    if 'm_id' in request.session:
        ...
        project_name = project.objects.filter(
            designation=Project_man).filter(department=did)
        Project_man_data = user_registration.objects.filter(
            designation=Project_man).filter(department=did).order_by("-id")
...
def MAN_promanattendsort(request, id):
    if 'm_id' in request.session:
        ...
        if request.method == "POST":
            ...
            mem1 = attendance.objects.filter(
                date__range=[fromdate, todate]).filter(user_id=id)
...
def BRadmin_trainerstable(request, did):
    if 'Adm_id' in request.session:
        ...
        trainers_data = user_registration.objects.filter(
            designation=Trainer).filter(department=did)
...
def BRadmin_trainingteam1(request, id):
    if 'Adm_id' in request.session:
        ...
        num = user_registration.objects.filter(
            designation=Trainee).filter(team=id).count()
...
def BRadmin_trainingprofile(request, id):
    if 'Adm_id' in request.session:
        ...
        num = trainer_task.objects.filter(
            user=user).filter(task_status='1').count()
...
def MAN_trainerstable(request, id):
    if 'm_id' in request.session:
        ...
        Trainer = designation.objects.get(designation='Trainer')
        trainers_data = user_registration.objects.filter(
            designation=Trainer).filter(department=id).order_by("-id")
        topics = topic.objects.all()
...
def MAN_trainingprofile(request, id):
    if 'm_id' in request.session:
        ...
        user = user_registration.objects.get(id=id)
        num = trainer_task.objects.filter(
            user=user).filter(status='Completed').count()
...
def BRadmin_View_Trainers(request, id, did):
    if 'Adm_id' in request.session:
        ...
        trainer = designation.objects.get(id=id)
        userreg = user_registration.objects.filter(
            designation=trainer).filter(department=did).order_by("-id")
...
def BRadmin_ViewTrainerattendancesort(request, id):
    if 'Adm_id' in request.session:
        ...
        if request.method == "POST":
            fromdate = request.POST.get('fromdate')
            todate = request.POST.get('todate')
            adata = attendance.objects.filter(
                date__range=[fromdate, todate]).filter(user_id=id)
...
def admin_emp(request):
    ...
    user = user_registration.objects.filter(
        designation_id=desig_id).filter(department_id=dept_id)
    print(dept)
    print(desi)
...
def man_desi(request):
    ...
    Desig = designation.objects.filter(
        ~Q(designation="admin"), ~Q(designation="manager"))
...
def BRadmin_projects(request, id):
    if 'Adm_id' in request.session:
        if request.session.has_key('Adm_id'):
            ...
            return redirect('/')
        Adm = user_registration.objects.filter(id=Adm_id)
        Num = project.objects.filter(~Q(status='Completed'), ~Q(
            status='Rejected')).filter(department=id).count()
        num = project.objects.filter(
            status='Completed').filter(department=id).count()
        project_details = project.objects.all()
...
def BRadmin_proj_list(request, id):
    if 'Adm_id' in request.session:
        ...
        project_details = project.objects.filter(
            ~Q(status='Completed'), department_id=id)
...
def BRadmin_proj_mangrs2(request, id):
    if 'Adm_id' in request.session:
        ...
        dept_id = project_details.department_id
        user_det = user_registration.objects.filter(
            designation_id=proj1).order_by("-id")
...
def BRadmin_developers(request, id):
    if 'Adm_id' in request.session:
        ...
        project_task = project_taskassign.objects.filter(
            project_id=project_details).filter(tl_id=id)
...
def MAN_projects(request, id):
    if 'm_id' in request.session:
        ...
        mem = user_registration.objects.filter(id=m_id)
        Num = project.objects.filter(
            status='accepted').filter(department=id).count()
        num = project.objects.filter(
            status='completed').filter(department=id).count()
        project_details = project.objects.all()
        depart = department.objects.get(id=id)
...
def BRadmin_ReportedissueShow(request, id, did):
    if 'Adm_id' in request.session:
        ...
        user = user_registration.objects.filter(
            designation=designations).filter(department=did).order_by("-id")
...
def MAN_givetask(request):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            print(sc1, sc2)
            catry = tasks(tasks=sc4, files=ogo, description=sc7, startdate=sc5,
                          enddate=sc6, department_id=sc1, designation_id=sc2, user_id=sc3)
            catry.save()
        dept = department.objects.all()
        desig = designation.objects.all()
...
def MAN_taskemployee(request):
    if 'm_id' in request.session:
        ...
        dept_id = request.GET.get('dept_id')
        desig_id = request.GET.get('desig_id')
        emp = user_registration.objects.filter(
            designation_id=desig_id).filter(department_id=dept_id)
        print(emp)
...
def BRadmin_viewprojectform(request):
    if 'Adm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            sc6 = request.POST['end']
            progress = 0
            catry = project_taskassign(
                project_id=sc3, description=sc4, startdate=sc5, enddate=sc6, extension='0')
            catry.save()
...
def BRadmin_seradmintraineedesi1(request):
    if 'Adm_id' in request.session:
        ...
        Adm = user_registration.objects.filter(id=Adm_id)
        dept_id = request.GET.get('dept_id')
        Desig = designation.objects.filter(~Q(designation='admin'), ~Q(
            designation='manager'), ~Q(designation='account'))
        print(Desig)
...
def man_delete(request, id):
    ...
    try:
        ...
        messages.success(
            request, "  Error Occured: It can't be deleted, it used as ForeignKey Constrain !!")
        return redirect('man_newdept')
    ...
    if request.method == 'POST':
        try:
            ...
            if (a.end_date < a.start_date):
                ...
                a.save()
                qr = qrcode.make(
                    "https://infoxcore.com/admin_code?id=" + str(a.id))
...
def man_registration_form(request):
    ...
    if request.method == 'POST':
        ...
        qr.save(settings.MEDIA_ROOT + "/images" +
                "//" + "offer"+str(y1.id) + ".png")
        ...
        qr1.save(settings.MEDIA_ROOT + "/images" +
                 "//"+"re" + str(y2.id) + ".png")
        ...
        qr2 = qrcode.make(
            "https://infoxcore.com/experienceletter/" + str(y3.id))
        qr2.save(settings.MEDIA_ROOT + "/images" +
                 "//"+"exp" + str(y3.id) + ".png")
...
def projectmanager_assignproject(request):
    if 'prid' in request.session:
        ...
        tes = user_registration.objects.filter(
            department_id=desi_id.department_id, designation_id=t.id)
        spa = user_registration.objects.filter(
            department_id=desi_id.department_id, designation_id=d.id)
        pvar = project.objects.filter(
            Q(status="accepted") | Q(status="assigned"))
...
def projectmanager_projectstatus(request, id):
    if 'prid' in request.session:
        ...
        var = project_taskassign.objects.filter(
            project_id=id, subtask__isnull=True).order_by('-id')
...
def projectmanager_projectlist(request):
    if 'prid' in request.session:
        if request.session.has_key('prid'):
            ...
            return redirect('/')
        pro = user_registration.objects.filter(id=prid)
        var = project.objects.filter(Q(status="accepted") | Q(
            status="assigned") | Q(status="completed")).order_by("-id")
...
def proMAN_rejectproj(request, id):
    ...
    if 'prid' in request.session:
        ...
        if request.method == 'POST':
            ...
            pro_sts = project.objects.filter(id=id).update(
                user_reason=user_reason, status='rejected', rejectdate=rejectdate)
...
def projectmanager_accepted_projects(request):
    if 'prid' in request.session:
        if request.session.has_key('prid'):
            ...
            return redirect('/')
        projects = project.objects.filter(~Q(status="Completed"), ~Q(
            status="completed"), ~Q(status="rejected"))
...
def projectmanDev(request):
    if 'prid' in request.session:
        ...
        man = user_registration.objects.filter(designation_id=des_id.id).filter(
            department_id=pro_dep.department_id, status="active").order_by("-id")
...
def pro_allocatetl(request):
    if 'prid' in request.session:
        ...
        devs = user_registration.objects.filter(
            designation_id=des_dev.id, status="active")
        tls = user_registration.objects.filter(
            designation_id=des_tl.id, status="active")
...
def projectMANattandance(request):
    if 'prid' in request.session:
        ...
        if request.method == "POST":
            fromdate = request.POST.get('fromdate')
            todate = request.POST.get('todate')
            prm1 = attendance.objects.filter(
                date__range=[fromdate, todate]).filter(user_id=prid)
...
def projectManager_tl(request):
    if 'prid' in request.session:
        ...
        tlfil = user_registration.objects.filter(
            designation_id=p.id, department_id=dept_id.department_id, status="active").order_by("-id")
...
def projectmanager_currentproject(request):
    if 'prid' in request.session:
        ...
        pro = user_registration.objects.filter(id=prid)
        var = project.objects.filter(
            department_id=proj.department_id, status="assigned")
...
def projectmanager_currentteam(request, id):
    if 'prid' in request.session:
        ...
        display = project_taskassign.objects.filter(
            project_id=id).values('tl_id').distinct()
...
def projectmanager_currenttl(request, id):
    if 'prid' in request.session:
        ...
        display = project_taskassign.objects.filter(
            project_id=id).values('project_id', 'tl_id').distinct()
...
def projectmanager_teaminvolved(request, id):
    if 'prid' in request.session:
        if request.session.has_key('prid'):
            ...
            return redirect('/')
        pro = user_registration.objects.filter(id=prid)
        pri = project_taskassign.objects.filter(
            developer_id=id).order_by("-id")
...
def MANreportsuccess(request):
    if 'prid' in request.session:
        ...
        man = user_registration.objects.get(
            branch_id=pro1.branch_id, designation_id=design.id)
        if request.method == 'POST':
...
def TLprojects(request):
    if 'tlid' in request.session:
        ...
        display = project_taskassign.objects.filter(
            tl_id=tlid).values('project_id').distinct()
        ...
        if request.method == 'POST':
            ...
            for ins_screenshot in lst_screenshot:
                ...
                if ins_screenshot:
                    ...
                    str_img = fs.save(
                        ''.join(filter(str.isalnum, str(img_emp))), img_emp)
                    str_img_path = fs.url(
                        ''.join(filter(str.isalnum, str_img)))
                    lst_file.append(
                        '/media/'+''.join(filter(str.isalnum, str(img_emp))))
            ...
            display = project_taskassign.objects.filter(
                tl_id=tlid).filter(project_id=id)
            tasks = project_taskassign.objects.filter(
                project_id=id, tl_id=tlid, developer_id=tlid)
...
def tlsplittask(request, id):
    if 'tlid' in request.session:
        ...
        tester = tester_status.objects.all()
        mem = user_registration.objects.filter(id=tlid)
        sub = project_taskassign.objects.filter(
            ~Q(developer_id=tlid)).filter(project_id=splitid, tl_id=tlid)
...
def tlgivetask(request):
    if 'tlid' in request.session:
        ...
        time = datetime.now()
        tasks = project_taskassign.objects.filter(
            tl_id=tlid, project_id=splitid).values('task').distinct()
...
def TLattendancesort(request):
    if 'tlid' in request.session:
        ...
        if request.method == "POST":
            ...
            # mem1 = attendance.objects.raw('select * from app_attendance where user_id and date between "'+fromdate+'" and "'+todate+'"')
            mem1 = attendance.objects.filter(
                date__range=[fromdate, todate]).filter(user_id=tlid)
...
def devtlreported(request):
    if 'tlid' in request.session:
        ...
        var = reported_issue.objects.filter(
            reported_to_id=tlid).order_by("-id")
...
def TSproject(request):
    if 'usernametsid' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernamets) .filter(fullname=usernamets1)
        pros = project_taskassign.objects.filter(
            tester=usertsid).values('project_id').distinct()
...
def TSprojectdetails(request, pid):
    if 'usernametsid' in request.session:
        ...
        mem = user_registration.objects.filter(
            designation_id=usernamets) .filter(fullname=usernamets1)
        var = project_taskassign.objects.filter(
            project=pid, tester_id=usertsid).order_by('-id')
...
def Devattendancesort(request):
    if 'devid' in request.session:
        ...
        if request.method == "POST":
            ...
            todate = request.POST.get('todate')
            mem1 = attendance.objects.filter(
                date__range=[fromdate, todate]).filter(user_id=devid)
...
def DEVprojects(request):
    ...
    devp = project_taskassign.objects.filter(
        developer_id=devid).values('project_id').distinct()
...
def DEVtable(request, id):
    ...
    dev = user_registration.objects.filter(id=devid)
    devp = project_taskassign.objects.filter(
        project_id=id).filter(developer_id=devid).order_by("-id")
...
def accounts_emp_list(request, id, pk):
    if 'usernameacnt2' in request.session:
        ...
        use = user_registration.objects.filter(
            department_id=mem.id, designation=mem1)
        context = {'use': use, 'z': z, }
    ...
    if 'usernameacnt2' in request.session:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                context = {
                    'msg': 'Course already exists!!!....  Try to add another course', 'z': z}
...
def account_reported_issue(request):
    if 'usernameacnt2' in request.session:
        ...
        z = user_registration.objects.filter(id=usernameacnt2)
        n = reported_issue.objects.filter(
            reporter_id=usernameacnt2).order_by('-id')
...
def accounts_acntpay(request):
    if 'usernameacnt2' in request.session:
        ...
        names = acntspayslip.objects.filter(fromdate__range=(fdate, tdate), designation_id=desig_id, department_id=dept_id).values(
            'user_id__fullname', 'eno', 'user_id__account_no', 'user_id__bank_name', 'user_id__bank_branch', 'user_id__id', 'user_id__email', 'id')
        print(fdate)
        print(tdate)
        print(dept_id)
        print(desig_id)
...
def SuperAdmin_profile(request, id):
    if 'SAdm_id' in request.session:
        ...
        Trainer = designation.objects.get(designation='trainer')
        trcount = user_registration.objects.filter(
            designation=Trainer, branch_id=id).count()
...
def SuperAdmin_trainerstable(request, id):
    if 'SAdm_id' in request.session:
        ...
        trainers_data = user_registration.objects.filter(
            designation=Trainer).filter(department=id)
        topics = topic.objects.all()
